...
from src.models.models import AssetAdministrationShell, Submodel, Property, ValueType, SubmodelElementCollection, DataElementCategory
...
def create_aas() -> AssetAdministrationShell:
    """
    Creates and returns an AAS instance from the AASX file
    """
    aas = aas_metamodel_converter("./aas-static/YOUR_AAS_HERE.aasx")
...
def collect_variable_properties(elements: List[Union[Property, SubmodelElementCollection]]):
    result = []
    for elem in elements:
        if isinstance(elem, Property):
            if elem.category == DataElementCategory.VARIABLE:
                result.append(elem)
        elif isinstance(elem, SubmodelElementCollection):
            result.extend(collect_variable_properties(elem.value))
    return result
